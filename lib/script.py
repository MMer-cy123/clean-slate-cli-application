...
def home_page(current_user_id):
    ...
    console.print(
         '''Listed below are variety of services we provide to our esteemed clients.\nEnter a task id to choose
         and book a cleaning session with us''', style="bold green")
    console.print(table)
    selected_task_id = click.prompt("Please select task_id:", type=int)
    if selected_task_id in range(1, (cleaning_tasks[-1].task_id)):
         ...
         for item in cleaning_tasks:
              if selected_task_id == item.task_id:
                   #console.print(f"{item.cleaner.full_name}")
                   console.print(
                         f'''{item.task_description} service has been booked successfully!! {item.cleaner.full_name} will arrive at your
                      premises in the next hour. Thank you for choosing clean slate😁''', style="green")
...
@welcome.command()
@click.option('--email', '-n', prompt="Enter your email:")
...
def sign_up(name, email, password, contact_number):
    ...
    if valid_email:
        ...
        session.commit()
        session.close()
        click.secho(("Account has been created successfully"), fg="green")
